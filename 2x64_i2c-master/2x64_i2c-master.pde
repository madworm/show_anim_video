...
#define __twi_slave_address 0x10 // we send data to this slave address
#define __RED 0x01
#define __GREEN 0x02
#define __BLUE 0x03
...
void set_led_red(byte row, byte led, byte red) {
  ...
  if( row > ( 2*__max_row + 1 ) ) { return; } // row out of bounds
  if( row > __max_row ) { // row goes to i2c slave
    Wire.beginTransmission(__twi_slave_address);
      Wire.send(__RED);
      Wire.send(row);
      Wire.send(led);
      Wire.send(red);
